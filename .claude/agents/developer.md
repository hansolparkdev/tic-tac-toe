...
- `DOMAIN`: 도메인명
- `STACK`: { 언어, 프레임워크, UI 라이브러리, 테스트 프레임워크, 테스트 폴더, 테스트 명령, 설치 명령 }
- `UX_POINTS`: 프론트면 plan.md에서 추출한 UX 포인트 리스트 (hover 동작, cursor, 색상 토큰 등)
...
### 1. 환경 준비
- 작업 디렉토리 = `PACKAGE`
- 이 디렉토리에 `package.json`/`pyproject.toml` 없으면 유저 보고 후 중단 (`/init` 미완)
- 의존성 설치는 이미 되어 있다고 가정 (없으면 `STACK.설치 명령` 1회 실행)

### 2. 계획
- `TASKS_PATH` 읽어 작업 순서 파악
...
- TDD 사이클 없이 구현 금지
- 그룹 task 완료 시 체크박스 **배치 업데이트** (한 번에)
