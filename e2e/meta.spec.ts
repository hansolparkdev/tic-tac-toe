 ...
 * 10. 저장 포맷 손상 시 기본값
 * 11. 탭 종료 후 재오픈 — 새 context → sessionStorage 격리 (유일하게 newContext 필요)
...
test.describe('Tic-Tac-Toe Meta E2E', () => {
  // Playwright 기본 `{ page }` 픽스처는 이미 테스트별로 격리된 context를 제공.
  // sessionStorage도 자동 격리되므로 별도 초기화 불필요.
  ...
  // 1. 초기 표시
  test('1. 빈 세션에서 "0/0/0"과 X 카드 강조', async ({ page }) => {
  ...
  // 2. 차례에 따른 강조 이동
  test('2. X 착수 후 O 카드 강조', async ({ page }) => {
  ...
  // 3. X 승리 → X 스코어 +1
  test('3. X 승리 시 X 스코어 0→1 & 카운트업 클래스', async ({ page }) => {
  ...
  // 4. 무승부 → draw +1
  test('4. 무승부 시 draw 스코어 0→1', async ({ page }) => {
  ...
  // 6. 오버레이 클릭 취소
  test('6. 다이얼로그 오버레이 클릭 시 취소 (스코어 유지)', async ({ page }) => {
    await playSequence(page, X_WIN_SEQ)
    await expect(scoreValueX(page)).toHaveText('1')
  ...
  // 8. 포커스 트랩
  test('8. 다이얼로그 내 Tab 반복 → [취소] ↔ [리셋] 순환', async ({ page }) => {
    await openDialogBtn(page).click()
    await expect(dialogCancelBtn(page)).toBeFocused()
  ...
  // 10. 저장 포맷 손상 시 기본값
  test('10. sessionStorage 손상 포맷 → 기본값으로 정상 동작', async ({ page }) => {
  ...
  // 11. 탭 종료 후 재오픈 — 이 시나리오만 새 context 시뮬레이션이 시맨틱상 필요
  ...
  // 12. 카드 접근 가능 텍스트
  test('12. 카드 aria-label "X N승", "O N승", "무승부 N"', async ({ page }) => {
